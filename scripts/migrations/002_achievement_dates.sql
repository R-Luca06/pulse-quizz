-- ─────────────────────────────────────────────────────────────────────────────
-- Migration 002 — Dates de déblocage des achievements dans get_public_profile
...
BEGIN
  ...
  -- Ids uniquement (pour BadgePicker côté client)
  SELECT array_agg(ua.achievement_id ORDER BY ua.unlocked_at)
  INTO v_achievements
  FROM user_achievements ua
  WHERE ua.user_id = v_user_id;

  -- Id + date de déblocage (pour le tooltip de la vitrine)
  SELECT json_agg(
    json_build_object('id', ua.achievement_id, 'unlocked_at', ua.unlocked_at)
    ORDER BY ua.unlocked_at
  )
  INTO v_achievement_dates
  FROM user_achievements ua
  WHERE ua.user_id = v_user_id;
  ...
  RETURN jsonb_build_object(
    ...
    'achievement_dates', COALESCE(v_achievement_dates, '[]'::jsonb)
  );
END;
$$;
