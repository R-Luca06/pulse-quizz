...
export default function StatsPage({ onBack }: Props) {
  ...
  // Jouées en premier, puis non jouées
  const sorted = [
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center overflow-y-auto bg-game-bg px-4 py-6 sm:py-10">
      ...
      <div className="pointer-events-none fixed inset-0">
        <div className="absolute left-1/2 top-1/3 h-[300px] w-[300px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-neon-violet/8 blur-3xl" />
      </div>
      ...
      <div className="relative z-10 flex w-full max-w-lg flex-col gap-6">
        ...
        >
          ...
          {/* Cards catégories */}
          <motion.div variants={fadeUp} className="flex flex-col gap-2 pb-8">
            <p className="mb-1 text-[10px] font-bold uppercase tracking-widest text-white/30">Par catégorie</p>
            {sorted.map((cat) => {
              const s = cat.stats
